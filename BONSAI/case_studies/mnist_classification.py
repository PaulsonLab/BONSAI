...
if __name__ == '__main__':
    ...
    if adversary_plot:
        ...
        for i in range(4): # Make this 5 for all rows
            for j in range(4):
                ax = axes[i,j]
                ...
                adversary = 5*np.random.rand(2, 8)
                total_adversary[2*(i + 1 ) - 2:2*(i +1 ),:] = adversary 
                    
                #if i == 0:
                #     total_adversary[0] = adversary[0]
                #     total_adversary[2] = adversary[1]
                    
                # if i == 1:
                #     total_adversary[1] = adversary[0]
                #     total_adversary[3] = adversary[1]
                    
                # if i == 2:
                #     total_adversary[4] = adversary[0]
                #     total_adversary[6] = adversary[1]
                    
                # if i == 3:
                #     total_adversary[5] = adversary[0]
                #     total_adversary[7] = adversary[1]                    
                    
                if i == 0:
                    ax.set_title("Test: %i" %j)
                
                img = image[j] + total_adversary 
                ax.set_axis_off()
                ax.imshow(img, cmap=plt.cm.gray, interpolation="lanczos")
                
            plt.savefig("train_fig_one.svg", format='svg', bbox_inches='tight')
        ...
        for j in range(4):
            img = image[j]  
            axes[j].set_axis_off()
            axes[j].imshow(img, cmap=plt.cm.gray, interpolation="lanczos")
            axes[j].set_title("Label: %i" %j)
          
        #plt.title('Labels for Training')
        #plt.savefig("label_fig.svg", format='svg', bbox_inches='tight')
